...
case "$arch" in
	...
	arm64)
		...
		;;
	*)
		die "unsupported Debian architecture: $arch; current autosnap releases publish linux amd64 and arm64 .deb assets only"
		;;
esac
...
asset_name="$(
	awk -v pattern="autosnap_.*_linux_${asset_arch}[.]deb$" '$2 ~ pattern { print $2 }' "$checksums_file"
)"

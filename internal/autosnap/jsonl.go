...
type checkpointJSONLOptions struct {
	...
	PatchStatus        map[string]checkpointPatchStatus
...
func checkpointJSONRows(ctx context.Context, repoRoot string, checkpoints []checkpointInfo, opts checkpointJSONLOptions) ([]map[string]any, error) {
	...
	for _, checkpoint := range checkpoints {
		...
		if opts.IncludePatchStatus {
			status, ok := opts.PatchStatus[checkpoint.Ref]
			if !ok {
		...
		mark := opts.Marks[checkpoint.Ref]
		if mark.Mark == "" {
			mark.Mark = checkpointMarkStateUnmarked
		}
		row["mark"] = mark.Mark
		if strings.TrimSpace(mark.Reason) != "" {
			row["reason"] = strings.TrimSpace(mark.Reason)
		}

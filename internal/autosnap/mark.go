...
func newMarkCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "mark (--bad|--good|--unmark) <checkpoint-or-range>",
		Short: "Mark checkpoints unmarked, good, or bad",
		Long: strings.TrimSpace(`Mark one checkpoint or an inclusive checkpoint range unmarked, good, or bad.
...
or one of these current-branch history selectors:
  ...
  last-N`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if modeCount != 1 {
				return fmt.Errorf("mark requires exactly one of --bad, --good, or --unmark")
			}
	...
	cmd.Flags().BoolVar(&unmark, "unmark", false, "Remove explicit good or bad marks from selected checkpoints")

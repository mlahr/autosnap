...
new flags. Detailed restart progress is appended to the autosnap daemon log.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			replacementProcess, err := startAutosnapDetachedWithMerge(repoRoot, cfg.Check, cfg.MsgSourceCmd, cfg.MsgBodySourceCmd, cfg.NoteCommand, cfg.NoteRef, cfg.PostCheckpointCommand, cfg.IdleSeconds, cfg.SnapshotMode, cfg.CommitMode, cfg.CommitMergeCommits, cfg.Watch.Mode, cfg.Watch.PollInterval, cfg.LogMaxBytes, runToken, runState.StartConfigFlags)
			if err != nil {
...
func resolveRestartConfig(repoRoot string, runState autosnapRunState) (autosnapConfig, bool, error) {
	...
	overrides := autosnapConfigOverrides{
		values: autosnapConfig{
			...
			CommitMergeCommits:    runState.CommitMergeCommits,
